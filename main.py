...
def build_main_keyboard(uid):
    ...
    if uid == OWNER_ID or user.get("is_reseller"):
        buttons.append([KeyboardButton("👑 Panel Admin")])
    
    return ReplyKeyboardMarkup(keyboard=buttons, resize_keyboard=True)

# === USERBOT ENGINE (Diimport dari userbot.py) ===
...
async def addprem_cmd(client, message):
    """Owner command: Berikan akses (format: /addprem USER_ID TIER HARI)"""
...
async def addseller_cmd(client, message):
    """Owner command: Jadikan user sebagai reseller"""
    if message.from_user.id != OWNER_ID:
        return await message.reply_text("❌ Owner only!")
    ...
    try:
        ...
        exp = (datetime.now() + timedelta(days=999)).isoformat()
        db["users"][uid]["expired"] = exp
        save_db(db)
